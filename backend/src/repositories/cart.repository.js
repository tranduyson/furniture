...
const removeCartItem = async (cartItemId, cartId) => {
  await pool.execute(`DELETE FROM cart_items WHERE id = ? AND cart_id = ?`, [cartItemId, cartId]);
};

const updateCartItemQuantity = async (cartItemId, cartId, quantity) => {
  await pool.execute(`UPDATE cart_items SET quantity = ? WHERE id = ? AND cart_id = ?`, [quantity, cartItemId, cartId]);
};
