...
const removeItemFromCart = async (userId, sessionId, cartItemId) => {
  const cart = await ensureCart(userId, sessionId);
  await cartRepository.removeCartItem(cartItemId, cart.id);
  return await fetchCart(userId, sessionId);
};

const updateItemQuantity = async (userId, sessionId, cartItemId, quantity) => {
  const cart = await ensureCart(userId, sessionId);
  if (quantity <= 0) {
    await cartRepository.removeCartItem(cartItemId, cart.id);
  } else {
    await cartRepository.updateCartItemQuantity(cartItemId, cart.id, quantity);
  }
  return await fetchCart(userId, sessionId);
};
